...
var subscribe = function(){  
 ...
 // connect to MongoDB
  require('mongodb').MongoClient.connect('mongodb://localhost/app', function(err, db){
   ...
   // make sure you have created capped collection "messages" on db "test"
    db.collection('tweets', function(err, coll) {
     ...
     // seek to latest object
      var seekCursor = coll.find(filter)//.sort({$natural: -1})
        .limit(1);
      ...
      seekCursor.nextObject(function(err, latest) {
       ...
       // create stream and listen
        var stream = coll.find(filter, cursorOptions)//.sort({$natural: -1})
            .stream();
